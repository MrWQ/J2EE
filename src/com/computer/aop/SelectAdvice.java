...
public class SelectAdvice {
     ...
     *
     */
    public void countNumber(List<ComputerModel> bindParam) {
        if (CollectionUtils.isEmpty(bindParam)) {
            logger.info("AfterReturn:统计增强方法:本次未查询出数据!");
            return;
        }
        String string = String.format("AfterReturn:本次查询出%s条记录", bindParam.size());
        logger.info(string);
    }
     ...
     *            绑定业务方法的返回值,也就是将select*这些方法的返回值作为这个'增强'方法的入参
     */
    public void encryptMessage(List<ComputerModel> bindParam) {
        if (CollectionUtils.isEmpty(bindParam)) {
            logger.info("AfterReturn:加密增强方法:本次未查询出数据!");
            return;
        }
        for (ComputerModel studentModel : bindParam) {
            if (null != studentModel.getCpu()) {
                studentModel.setCpu("i加密");
            }
        }
